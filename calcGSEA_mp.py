...
Implementation based on Morten's code:
...
def gen_permutation_score_dist(n_permutations, n_genes, n_geneset_genes, posv, negv, morten_res=True):
    _distribution = []
    _pa = gen_permutation_matrix(permutations=n_permutations, n_genes=n_genes, n_geneset_genes=n_geneset_genes)
    for _p in _pa:
        _tmp_score, _ = calc_GSEA_score(n_genes=n_genes, ovlp=_p, posv=posv, negv=negv, morten_res=morten_res)
        _distribution.append(_tmp_score)
    return _distribution


def calc_GSEA_score(n_genes, ovlp, posv, negv, morten_res=True):
    _result = None
    ...
    for i in range(len(ovlp)):
        ...
        _previous = _scrv[i]
    if morten_res:
        # This is how Morten calcs the GSEA score
...
def calc_GSEA_score_for_gset(data: Union[pd.DataFrame, np.ndarray],
    ...
    :param morten_res: return max + min GSEA score if True else return max deviation from 0
    ...
    for ri, rd in _df.iterrows():
        ...
        results[ri], scrvs[ri] = calc_GSEA_score(n_genes=N, ovlp=ovlp, posv=posv, negv=negv, morten_res=morten_res)
    _distribution = None
    if n_permutations is not None and isinstance(n_permutations, int):
        _distribution = gen_permutation_score_dist(n_permutations=n_permutations, n_genes=N, n_geneset_genes=SIG,
                                                   posv=posv, negv=negv, morten_res=morten_res)
...
def apply_z_score(df, veracyte=False):
    if veracyte:
        df = df.apply(zscore)
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument("--veracyte", dest='veracyte', action='store_true',
                        help="use this argument if input data is veracyte microarray data")
    ...
    parser.set_defaults(veracyte=False)
        ...
        if args.n_procs > 1:
            ...
            for gset_n, gset_g in sig_d.items():
                ...
                for _df_chunk in _df_split:
                    mp_args.append((gset_n, {'data': _df_chunk, 'sig_genes': gset_g, 'mean_centering': False,
                                           'morten_res': True, 'n_permutations': 1000}))
            ...
            for gset_n, gset_g in sig_d.items():
                ...
                _data, _scrvs, _dist = calc_GSEA_score_for_gset(data=df_n_z, sig_genes=gset_g,
                                                                mean_centering=False, morten_res=True,
                                                                n_permutations=1000)
